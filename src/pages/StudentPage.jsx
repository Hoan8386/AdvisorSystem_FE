...
export const StudentPage = () => {
  ...
  const handleMarkAllAsRead = async () => {
    try {
      ...
      if (res?.success) {
        toast.success(res?.message || "Đã đánh dấu tất cả thông báo là đã đọc");
        fetchNotifications();
        setUnreadCount(0);
      } else {
      ...
      toast.error(error?.message || "Lỗi khi đánh dấu thông báo");
    } finally {
  ...
  const handleCloseModal = () => {
    ...
    // Load lại danh sách thông báo khi đóng modal
    fetchNotifications();
    fetchUnreadCount();
  };
  ...
  const handleSubmitResponse = async () => {
    ...
    if (!selectedNotification?.notification_id) {
      toast.error("Lỗi: Không tìm thấy ID thông báo");
      return;
    }
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button
                ...
                className="px-6 py-3 bg-gradient-to-r from-orange-500 to-yellow-500 text-white rounded-xl font-semibold shadow-lg shadow-orange-200 hover:shadow-xl transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
              >
        ...
        <Spin spinning={detailLoading}>
          {selectedNotification && (
            <div className="flex flex-col gap-5">
              ...
              {/* My Question & Response Section */}
              {selectedNotification.my_response ? (
                <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border-2 border-indigo-300 rounded-2xl p-5">
                  <h4 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                    💬 Câu hỏi & Phản hồi
                  </h4>
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold ${
                          selectedNotification.my_response.status === "resolved"
                            ...
                            : selectedNotification.my_response.status ===
                              "pending"
                    ...
                    <div className="bg-white rounded-xl p-4 border-2 border-green-200">
                      <div className="flex items-start gap-3 mb-3">
                        <div className="w-10 h-10 bg-gradient-to-r from-green-500 to-emerald-500 rounded-full flex items-center justify-center text-white font-bold">
                          {selectedNotification.my_response.advisor_user?.full_name?.charAt(
                            0
                          ) || "G"}
                        </div>
                        <div className="flex-1">
                          <div className="font-bold text-gray-900">
                            {selectedNotification.my_response.advisor_user
                              ?.full_name || "Cố vấn"}
                          </div>
